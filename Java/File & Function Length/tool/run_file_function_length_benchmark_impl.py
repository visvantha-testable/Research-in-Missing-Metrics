"""Lizard File & Function Length benchmark execution helpers (Java)."""
...
def build_long_methods(metrics_df: pd.DataFrame) -> pd.DataFrame:
    if metrics_df.empty:
        return pd.DataFrame(columns=LONG_METHOD_COLUMNS)
    rows: list[dict[str, Any]] = []
...
def run_pipeline(repo: Path, output: Path) -> dict[str, Any]:
    ...
    long_methods_df.to_csv(output / "long_methods.csv", index=False)
    ...
    return {
        "benchmark_ready": (
            ...
            and max_function_length > LONG_METHOD_THRESHOLD
