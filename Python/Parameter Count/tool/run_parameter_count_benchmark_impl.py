"""Lizard Parameter Count benchmark execution helpers."""
...
def save_python_inventory(python_files: list[Path], output: Path) -> None:
    rows = [
...
def run_pipeline(repo: Path, output: Path) -> dict[str, Any]:
    ...
    return {
        "benchmark_ready": len(python_files) > 0 and not metrics_df.empty and max_param >= 8,
