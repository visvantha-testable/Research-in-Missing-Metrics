"""Lizard Parameter Count benchmark execution helpers (C)."""
...
def build_long_parameter_list(metrics_df: pd.DataFrame) -> pd.DataFrame:
    if metrics_df.empty:
        return pd.DataFrame(columns=["file", "function", "parameter_count", "status"])
...
def run_pipeline(repo: Path, output: Path) -> dict[str, Any]:
    ...
    return {
        "benchmark_ready": len(c_files) > 0 and not metrics_df.empty and max_param >= 8,
        "c_files": len(c_files),
