"""Generate lizard_parameter_count_extraction.ipynb for C repositories."""
...
NOTEBOOK = METRIC_ROOT / "lizard_parameter_count_extraction.ipynb"
...
EXCLUDED_DIR_NAMES = {
    ".git", "build", "bin", "dist", "deps", "tests", "docs", "third_party", "vendor",
}
LIZARD_EXCLUDE_PATTERNS = [
    "*/.git/*", "*/build/*", "*/bin/*", "*/dist/*", "*/deps/*",
    "*/tests/*", "*/docs/*", "*/third_party/*", "*/vendor/*",
]
LIZARD_RAW_COLUMNS = [
...
def discover_c_files(repo_path: Path) -> list[Path]:
    ...
    for path in repo_path.rglob("*"):
        if not path.is_file():
            continue
        if path.suffix.lower() not in C_FILE_EXTENSIONS:
            continue
...
def build_long_parameter_list(metrics_df: pd.DataFrame) -> pd.DataFrame:
    ...
    return pd.DataFrame(rows, columns=["file", "function", "parameter_count", "status"])
...
cells = [
    md(
        "# Lizard Parameter Count — Raw Output Extraction (C)\n\n"
        "This notebook analyzes **C repositories** with **Lizard** and captures complete raw tool output "
        "for Parameter Count, Cyclomatic Complexity (CCN), Function Length, NLOC, Token Count, and Maximum Nesting Depth.\n\n"
        "**Default benchmark repository:** [redis/redis](https://github.com/redis/redis)\n\n"
        "> **Note:** **Parameter Count is directly emitted by Lizard** through the `PARAM` column in CSV output. "
        "No derivation is required for this metric."
    ),
    ...
    code(
        ...
        "print(f\"C source files: {REPO_STATS['c_file_count']:,}\")"
    ),
    ...
    md(
        ...
        "Run `lizard -l c --csv` and nesting analysis with `-ENS` (equivalent to `-E NS`).\n\n"
    ...
    md(
        ...
        "**Direct metric** (emitted by Lizard via the `PARAM` column):\n\n"
        "```text\n"
    ...
    code(
        ...
        "logger.info(f'Parameter Count (max PARAM)={MAX_PARAM} (directly from Lizard)')\n"
        "display(pd.read_csv(PARAM_SUMMARY_CSV))"
    ),
    md("## Section 10 — Long Parameter List Detection\n\nFlag functions where `Parameter_Count > 5`."),
    code(
    ...
    md("## Section 11 — Complexity Summary"),
    code(
    ...
    code(
        ...
        "display(summary_df)\n\n"
        "deliverables = [\n"
        "    CONSOLE_PATH, LIZARD_OUTPUT_CSV, METRICS_CSV, PARAM_SUMMARY_CSV,\n"
        "    LONG_PARAM_CSV, COMPLEXITY_CSV, INVENTORY_CSV, ERROR_LOG_PATH,\n"
        "]\n"
        "print('\\nDeliverables:')\n"
